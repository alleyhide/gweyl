...
#if 0    
matrix gweyl::inverse_cartan_matrix(const root_space& V){
	matrix P;
	unsigned n = V.dimension();
	P.resize(n, n);
	...
	case 'E':
		...
		case 8:
			...
			break;
		default:
			throw domain_error("unknown type E: inverse_cartan_matrix");
		}
		break;
	case 'F':
            P(0,0)=rational(2); P(0,1)=rational(3); P(0,2)=rational(2); P(0,3)=rational(1);
	...
	case 'G':
		...
		break;
	default:
		throw domain_error("unknown type: inverse_cartan_matrix");
	}
